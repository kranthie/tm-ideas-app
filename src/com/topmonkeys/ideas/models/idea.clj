...
(defn add-idea
  ...
  (with-mongo db/connection
    (insert! ideas-coll {:id (db/get-next-id ideas-coll) :title title 
                         :handle handle :username username :description description 
...
(defn add-comment
  ...
  (with-mongo db/connection
    ...
    (fetch-and-modify ideas-coll {:handle handle} {:$push {:comments {:id (db/get-next-id (str ideas-coll "_comments")) :comment comment :username username :created-at (to-long (now))}}} :return-new? true)))
...
(defn update-idea
  "Updates an idea."
  [{:keys [_id title description handle]}]
  (with-mongo db/connection
    (fetch-and-modify ideas-coll {:_id _id} {:$set {:title title :handle handle :description description :updated-at (to-long (now))}} :return-new? true)))
